...
interface SessionState {
  ...
  updateTosnaSchedule: (
    breweryId: string | null | undefined,
    sessionId: string | null | undefined,
    updatedAdditions: TosnaAddition[],
  ) => Promise<void>;
  splitBrewSession: (payload: {
    breweryId: string;
    parentSessionId: string;
    splits: { volumeLiters: number; namePostfix: string }[];
  }) => Promise<void>;
  updateSessionStatus: (
    breweryId: string | null | undefined,
    sessionId: string | null | undefined,
    newStatus: BrewSessionStage,
    actualOg?: number,
  ) => Promise<void>;
  deleteSession: (
    breweryId: string | null | undefined,
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  fetchSessions: async (breweryId) => {
    ...
    try {
      ...
      const sessions = (data || []).map((row) => {
        ...
        try {
          parsedSteps =
            typeof row.session_steps === 'string'
              ? JSON.parse(row.session_steps)
              : row.session_steps || [];
        } catch (e) {
        ...
        try {
          parsedIngredients =
            typeof row.session_ingredients === 'string'
              ? JSON.parse(row.session_ingredients)
              : row.session_ingredients || [];
        } catch (e) {
          console.warn('Failed to parse ingredients', e);
        }
        try {
          parsedTosna =
            typeof row.tosna_schedule === 'string'
              ? JSON.parse(row.tosna_schedule)
              : row.tosna_schedule || null;
        } catch (e) {
          console.warn('Failed to parse tosna', e);
        }
        ...
        return {
          ...
          batchSizeLiters:
            row.actual_batch_size_liters || row.batch_size_liters || 0,
          targetOg: row.actual_original_gravity || row.target_og || 1.0,
          targetFg: row.actual_final_gravity || row.target_fg || 1.0,
  ...
  fetchSessionById: async (breweryId, sessionId) => {
    ...
    try {
      ...
      try {
        parsedSteps =
          typeof sessionData.session_steps === 'string'
            ? JSON.parse(sessionData.session_steps)
            : sessionData.session_steps || [];
      } catch (e) {
        console.warn('Failed to parse steps', e);
      }
      try {
        parsedIngredients =
          typeof sessionData.session_ingredients === 'string'
      ...
      try {
        parsedTosna =
          typeof sessionData.tosna_schedule === 'string'
            ? JSON.parse(sessionData.tosna_schedule)
            : sessionData.tosna_schedule || null;
      } catch (e) {
        console.warn('Failed to parse tosna', e);
      }
      ...
      const mappedSession: BrewSession = {
        ...
        batchSizeLiters:
          ...
          0,
        targetOg:
          sessionData.actual_original_gravity || sessionData.target_og || 1.0,
        targetFg:
          sessionData.actual_final_gravity || sessionData.target_fg || 1.0,
  ...
  updateSteps: async (breweryId, sessionId, newSteps) => {
    ...
    if (currentSession)
      set({
        currentSession: { ...currentSession, sessionSteps: newSteps || [] },
      });

    try {
      await supabase
        .from('brew_sessions')
        .update({ session_steps: newSteps })
        .eq('id', sessionId)
        .eq('brewery_id', breweryId);
    } catch (err: unknown) {
      set({
        error: err instanceof Error ? err.message : 'Failed to update steps',
      });
    }
  },
  ...
  updateSessionStatus: async (breweryId, sessionId, newStatus, actualOg) => {
    ...
    if (currentSession) {
      set({
        currentSession: { ...currentSession, ...stateUpdate } as BrewSession,
      });
    }
  ...
  addLogToSession: async (breweryId, sessionId, newLog) => {
    ...
    const usedFg = currentSession?.actualFg || currentSession?.targetFg || 1.0;

    if (
      usedOg &&
      newLog.sg !== null &&
      updatedTosna &&
      !updatedTosna.isCompressed
    ) {
    ...
    try {
      ...
      if (updatedTosna) {
        await supabase
          .from('brew_sessions')
          .update({ tosna_schedule: updatedTosna })
          .eq('id', sessionId)
          .eq('brewery_id', breweryId);
      }
    } catch (err: unknown) {
  ...
  updateTosnaSchedule: async (breweryId, sessionId, updatedAdditions) => {
    ...
    if (currentSession && updatedTosna) {
      set({
        currentSession: { ...currentSession, tosnaSchedule: updatedTosna },
      });
    }
    ...
    try {
      ...
      set({
        error: err instanceof Error ? err.message : 'Failed to update TOSNA',
      });
    }
  },
