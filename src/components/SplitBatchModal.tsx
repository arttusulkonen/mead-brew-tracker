...
export const SplitBatchModal: React.FC<SplitBatchModalProps> = ({ currentVolume, onClose, onSubmit }) => {
  ...
  const [splits, setSplits] = useState<{ volumeLiters: number; namePostfix: string }[]>([
    { volumeLiters: Number((currentVolume / 2).toFixed(1)), namePostfix: 'Part 1' },
    { volumeLiters: Number((currentVolume / 2).toFixed(1)), namePostfix: 'Part 2' }
  ]);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const totalSplitVolume = splits.reduce((sum, split) => sum + (Number(split.volumeLiters) || 0), 0);
  const isValid = totalSplitVolume > 0 && totalSplitVolume <= currentVolume && splits.every(s => s.namePostfix.trim().length > 0 && s.volumeLiters > 0);
    ...
    <div className="bottom-sheet-overlay" onClick={onClose}>
      <div className="bottom-sheet" onClick={e => e.stopPropagation()}>
        ...
        <div className="bottom-sheet__context">
          {t('Current Volume')}: <strong>{currentVolume} {t('L')}</strong>
          <br />
          {t('Allocated')}: <strong style={{ color: totalSplitVolume > currentVolume ? 'var(--color-error)' : 'inherit' }}>{totalSplitVolume.toFixed(1)} {t('L')}</strong>
        </div>

        <form className="bottom-sheet__form" onSubmit={handleSubmit}>
          <div className="timeline__list">
            {splits.map((split, index) => (
              <div key={index} className="timeline__item timeline__item--active" style={{ padding: '12px', margin: '0', flexDirection: 'column', gap: '12px' }}>
          ...
          <button type="button" className="btn-secondary" onClick={handleAddSplit} disabled={isSubmitting || totalSplitVolume >= currentVolume}>
