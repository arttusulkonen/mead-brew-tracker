...
2. Preparation Staging: Generate separate, sequential steps for:
   - Step A: Honey Dissolution (Strict No-Boil limit: 40-45°C).
   - Step B: Must Aeration & Volume Adjustments (intense oxygenation) AND measuring original gravity (OG).
...
3. Fermentation Staging (TIMELINE LOGIC): 
   ...
   - For Step B (Degassing/Day 1 feeding), set duration to 1 day.
...
4. Additives & Conditioning Staging:
   ...
   - Flocculation & Stabilization: Generate separate steps for Cold Crash (2-4°C for 48-72h) and Racking (careful siphoning off the yeast cake).

# Stabilization & Carbonation
- For Sweet Still Meads/Ciders (targetFg > 1.006): DO NOT suggest priming sugar. Include a "Cold Crash" step, followed by chemical stabilization using Potassium Sorbate AND Potassium Metabisulfite for 24h BEFORE adding back-sweetening honey/sugar.
...
- EXPLAIN THE "WHY": Always briefly explain why a step is necessary (e.g., "We don't boil honey because it destroys delicate floral aromas", "Degas first, otherwise the powder will cause a foam explosion").
